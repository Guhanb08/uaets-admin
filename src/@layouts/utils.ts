...
/*   return matchedRoutes.some(route => {
    ...
    return matchedRoutes.some(route => {
      const isNameMatch = route.name === resolveRoutedName || route.meta.navActiveLink === resolveRoutedName
      const hasMatchingId =
      link.to && typeof link.to === 'object' && 'params' in link.to && link.to.params?.id &&
      router.currentRoute.value.params?.id
        ? link.to.params.id === router.currentRoute.value.params.id
        : true // If no id in link, only check the name
      return isNameMatch && hasMatchingId
    })
}
