...
const {
  mutate: repositionTag,
  loading: repositionTagFetching,
  error: repositionTagError,
} = useRepositionTagMutation();
...
const onDelete = async (confirm: boolean) => {
  ...
  await deleteTag({ id: selectedRow.value.id });

  if(!tagDeleteError.value) {
...
const onRowSelected = (args: any) => {
  selectedRow.value = args.data as Tag;
};
...
const onStatusChange = async (tag: Tag) => {
  const newStatus =
    tag.status === Status.Active ? Status.Inactive : Status.Active;
  await updateTag({ id: tag.id, data: { status: newStatus } });
  await refetchTags();
};
...
watch(
  [tagError, tagDeleteError, repositionTagError, updateTagError],
  (errors) => {
...
<template>
  <section>
    <VRow>
      <VCol cols="12">
        <VCard id="table-list" title="Tags">
          <VCardText>
            <VProgressLinear
              v-if="
                tagLoading ||
                tagDeleteLoading ||
                repositionTagFetching ||
                updateTagLoading
              "
              indeterminate
              height="3"
