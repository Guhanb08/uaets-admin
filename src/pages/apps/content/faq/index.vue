...
const {
  mutate: repositionFaq,
  loading: repositionFaqFetching,
  error: repositionFaqError,
} = useRepositionFaqMutation();
...
const {
  mutate: updateFaqStatus,
  loading: updateFaqStatusLoading,
  error: updateFaqStatusError,
} = useUpdateFaqStatusMutation();
...
const onDelete = async (confirm: boolean) => {
  ...
  await deleteFaq({ id: selectedRow.value.id });
  if(!faqDeleteError.value) {
...
const onRowSelected = (args: any) => {
  selectedRow.value = args.data as Faq;
};
...
const onStatusChange = async (faq: Faq) => {
  const newStatus =
    faq.status === Status.Active ? Status.Inactive : Status.Active;
...
watch(
  [faqError, faqDeleteError, repositionFaqError, updateFaqError],
  (errors) => {
...
<template>
  <section>
    <VRow>
      <VCol cols="12">
        <VCard id="table-list" title="Faq">
          <VCardText>
            <VProgressLinear
              v-if="
                faqLoading ||
                faqDeleteLoading ||
                repositionFaqFetching ||
                updateFaqLoading
              "
              indeterminate
              height="3"
